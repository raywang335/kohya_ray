...
def get_latents(vae, key_and_images, tokenizers, text_encoders, weight_dtype=torch.float32):
    ...
    for key, image in key_and_images:
        caption = open(key[:-4]+".txt", "r", encoding="utf-8").read().strip()
...
def main(args):
    ...
    def process_batch(is_last):
        for bucket in bucket_manager.buckets:
            if (is_last and len(bucket) > 0) or len(bucket) >= args.batch_size:
                latents, hid1s, hid2s, pools = get_latents(vae, [(key, img) for key, img, _, _ in bucket], tokenizers, text_encoders, weight_dtype)
                assert (
                ...
                for (image_key, _, original_size, crop_left_top), latent, hid1, hid2, pool in zip(bucket, latents, hid1s, hid2s, pools):
                    ...
                    npz_file_name = get_npz_filename_wo_ext(args.train_data_dir, image_key[:-4]+"_text", args.full_path, False, args.recursive)
    ...
    for data_entry in tqdm(data, smoothing=0.0):
        ...
        metadata[image_key]["npz_path"] = os.path.join(args.train_data_dir, os.path.basename(image_key)[:-4]) + ".npz"
        metadata[image_key]['text_latent'] = os.path.join(args.train_data_dir, os.path.basename(image_key)[:-4]+"_text") + ".npy"
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--min_bucket_reso", type=int, default=768, help="minimum resolution for buckets / bucketの最小解像度")
    parser.add_argument("--max_bucket_reso", type=int, default=1280, help="maximum resolution for buckets / bucketの最小解像度")
    parser.add_argument(
        "--bucket_reso_steps",
        type=int,
        default=64,
