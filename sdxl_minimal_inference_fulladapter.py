...
if __name__ == "__main__":
    ...
    sd_adapter, _ = sdxl_model_util.load_adapters_from_sdxl_checkpoint(sdxl_model_util.MODEL_VERSION_SDXL_BASE_V0_9, args.adapter_ckpt_path, "cpu", isfull=True)
    ...
    def generate_image(prompt, negative_prompt, seed=None, model_name=None, lineart_path=None):
        ...
        with torch.no_grad():
            ...
            lineart_img = lineart_img.resize((target_height, target_width))
            # crop_left = 0
            # crop_top = 0
            ...
            lineart_img = torch.FloatTensor(np.array(lineart_img) / 255.0 *2 -1).unsqueeze(0).unsqueeze(0).to(DEVICE, dtype=DTYPE) 
        ...
        cond_image = np.array(lineart_img.cpu().squeeze().unsqueeze(-1).repeat(1,1,3).numpy() * 255).astype("uint8")
        image = [np.concatenate([cond_image, image[i], ori_image[i]], axis=1) for i in range(image.shape[0])]
