...
from library.sdxl_t2i_adapter import SdxlT2IAdapter, SdxlT2IAdapterFull
...
@torch.no_grad() 
def inference_samples(unet, vae, controller, ori_image_path, text_embeddings, content_embeddings):
    # settings
    target_num = 1
    steps = 25
    guidance_scale = 9
    ...
    latents_shape = (cond_encoder_hidden_states1.shape[0], 4, controller[0].shape[-2], controller[0].shape[-1])
    latents = torch.randn(
        latents_shape,
        device="cpu",
        dtype=torch.float32,
    ...
    for i in range(target_num):
        ...
        sketch_image_path = os.path.join("/mnt/nfs/file_server/public/wangrui/sd_finetune_lion400/sd_xl_test/kohya_ss/T2Iadapter_lineart/lineart_data",os.path.basename(os.path.dirname(ori_image_path[i])), os.path.basename(ori_image_path[i]))
...
def train(args):
    ...
    if args.adapter_resume_path is None:
        sdxl_adapter = SdxlT2IAdapterFull()
    else:
        assert os.path.exists(args.adapter_resume_path), f"adapter_resume_path {args.adapter_resume_path} does not exist"
        sdxl_adapter = sdxl_train_util.load_xl_model(args, accelerator, 'adapter', weight_dtype, isfull=True)
    ...
    init_latent = torch.randn(4, 4, 128, 128)
    controller = None
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if True:
                ...
                fixed_crop_emb = torch.zeros((orig_size.shape[0], 2), dtype=orig_size.dtype, device=orig_size.device)
                fixed_tar_emb = 1024. *  torch.ones((orig_size.shape[0], 2), dtype=orig_size.dtype, device=orig_size.device)
                embs = sdxl_train_util.get_size_embeddings(orig_size, crop_size, target_size, accelerator.device).to(weight_dtype)
                with torch.no_grad():
                    fixed_embs = sdxl_train_util.get_size_embeddings(orig_size, fixed_crop_emb, fixed_tar_emb, accelerator.device).to(weight_dtype).detach()
                # concat embeddings
                ...
                infer_content_embeddings = fixed_embs
            ...
            if accelerator.sync_gradients:
                ...
                if args.log_image_every_n_steps is not None and global_step % args.log_image_every_n_steps == 0:
                    ...
                    if accelerator.is_main_process:
                        with torch.no_grad():
                            samples = inference_samples(unet, vae, controller, ori_img_name, infer_text_embedding, infer_content_embeddings)
