...
def main(args):
  ...
  processing_file = open("/mnt/nfs/file_server/public/wangrui/sd_finetune_lion400/sd_xl_test/df_data/ray_tools/processing_filtered_images_after_dect_opencv.txt", 'w')
  cache_file = open("/mnt/nfs/file_server/public/wangrui/sd_finetune_lion400/sd_xl_test/df_data/ray_tools/cache_filtered_images_after_dect_opencv.txt", 'w')
  # train_data_dir_path = Path(args.train_data_dir)
  image_paths = open(args.train_data_dir, 'r').readlines()
  ...
  for image_path in tqdm(image_paths):
    caption_path = os.path.join(args.text_dir, os.path.basename(os.path.dirname(image_path))+"xyz"+os.path.basename(image_path)[:-4]+"txt")
    if not os.path.exists(caption_path):
        processing_file.write(image_path)
        continue
    caption = open(caption_path).readline().strip()
    cache_file.write(image_path)
...
def setup_parser() -> argparse.ArgumentParser:
  ...
  parser.add_argument("--text_dir", type=str, default="", help="debug mode")
